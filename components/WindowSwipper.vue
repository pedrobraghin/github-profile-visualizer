...
<template>
  <div
    class="w-screen md:max-w-[80%] h-screen flex flex-col items-center justify-center"
  >
    <div class="mb-4 w-full flex gap-2 flex-col px-8 md:flex-row md:px-0">
      <button
        class="bg-blue-500 text-white p-2 rounded-lg w-full transition-all hover:opacity-75 h-full"
      ...
      <button
        class="bg-text-secondary-dark text-white p-2 rounded-lg w-full transition-all hover:opacity-75 h-full"
    ...
    <div class="relative w-full h-full overflow-x-hidden">
      <div
        v-if="showContent && position === 'left'"
      ...
      <div
        v-if="showContent && position === 'right'"
        class="w-full h-full flex items-center justify-center absolute transition-all duration-500"
      >
        <component :is="right" />
      </div>
